...
function hdr(t,p) {
  return '<header class="h" id="hdr"><div class="hi"><a href="/" class="l"><div class="li">\u{1F3E5}</div><div class="lt">'+t.titleShort+'<sm>'+t.osVer+'</sm></div></a><nav class="nv" id="mn"><a href="#depts">'+t.navDepts+'</a><a href="#branches">'+t.navBranches+'</a><a href="#doctors">'+t.navDoctors+'</a><a href="#ecosystem"><span style="font-size:.75rem;opacity:.6;">\u{1F310}</span> '+t.navEcosystem+'</a><button class="lb" id="lb">'+t.langBtn+'</button><a href="https://bsma.elfadil.com" target="_blank" class="btn bp bs">'+t.navBook+'</a></nav><button class="mb" onclick="document.getElementById(\'mn\').classList.toggle(\'op\');return false;">\u{2630}</button></div></header>';
}
...
function ecoSys(t) {
  return '<section class="se sea" id="ecosystem"><div class="co"><div class="sh an"><h2>'+t.ecosystemTitle+'</h2><div class="gl"></div><p>'+t.ecosystemSub+'</p></div><div class="ec"><h3 style="font-size:1.1rem;font-weight:700;color:var(--n);margin-bottom:16px;">\u{1F465} '+t.navPortal+'</h3><div class="ec-g">'+
    ...
    '  <a href="#digitalHealth" class="ec-c an d1"><div class="ec-badge ok pf">New</div><div class="ec-ic" style="background:linear-gradient(135deg,#10B981,#34D399)">\u{1F3E0}</div><div class="ec-n">'+t.hcTitle+'</div><div class="ec-d">'+t.hcSub+'</div></a>'+
    '  <a href="#digitalHealth" class="ec-c an d2"><div class="ec-badge ok pf">New</div><div class="ec-ic" style="background:linear-gradient(135deg,#0EA5E9,#38BDF8)">\u{1F3A5}</div><div class="ec-n">'+t.thTitle+'</div><div class="ec-d">'+t.thSub+'</div></a>'+
...
'\n        {key:"bsma_portal",icon:"\u{1F399}\uFE0F",name:"'+t.bsmaPortal+'"},' +
...
export function servePage(req) {
  ...
  var bgHtml = BR.map(function(b,i){
    ...
    return '<div class="cd bc an d'+(i%3+1)+'"><div class="bh"><h3>'+(A?b.nA:b.nE)+'</h3><div class="bcx">'+(A?b.aA:b.aE)+'</div></div><div class="bb"><a href="tel:'+p+'" class="btn bp bs">'+(A?'اتصل':'Call')+'</a><a href="/branches/'+slug+'" class="btn bw bs">'+(A?'المزيد':'Details')+'</a></div></div>';
  }).join('');
  var dogHtml = DO.map(function(d,i){
    var nm=A?d.nA:d.nE;
    var init=nm.replace('د. ','').replace('Dr. ','').charAt(0);
    return '<div class="cd dcc an d'+(i%4+1)+'"><div class="da">'+init+'</div><h3>'+nm+'</h3><div class="sp">'+(A?d.sA:d.sE)+'</div><a href="tel:'+p+'" class="btn bp bs">'+(A?'احجز':'Book')+'</a></div>';
  }).join('');
